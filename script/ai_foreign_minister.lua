...
local liInfluenceCap = 22 -- Cap based on total leadership, if below this do not influence at all
...
function ForeignMinister_Tick(minister)
    ...
    if math.mod( CCurrentGameState.GetAIRand(), 12) == 0 then
		if Utils.HasCountryAIFunction( minister:GetCountryTag(), "Call_ForeignMinister_Tick") then
			Utils.CallCountryAI(minister:GetCountryTag(), "Call_ForeignMinister_Tick", minister)							
		end
	end
...
function ForeignMinister_HandlePeace(minister)
	...
	local liTotalLeaderShip = ministerCountry:GetTotalLeadership():Get()
	local liDailyDiplomatic = math.floor((liTotalLeaderShip * 0.15) / 2)
	...
	for loTargetCountry in CCurrentGameState.GetCountries() do
		...
		and loTargetCountryTag:IsValid() then
			...
			local loReverseRelation = ai:GetRelation(loTargetCountryTag, ministerTag)
			...
			if lbIsMajor then
				...
				if ministerCountry:HasFaction() and not(loTargetCountry:HasFaction()) then
					...
					-- Make sure we exceed the influence cap if not bypass this code
					if liTotalLeaderShip > liInfluenceCap then
				...
				if not(ministerCountry:HasFaction()) then
					if not(loRelation:HasAlliance()) and tonumber(tostring(loRelation:GetValue():GetTruncated())) > 0 then
						local liScore = DiploScore_Alliance(ai, ministerTag, loTargetCountryTag, loTargetCountryTag)
						if liScore > 50 then
			...
			if loReverseRelation:HasMilitaryAccess() then
				local loAction = CMilitaryAccessAction(loTargetCountryTag, ministerTag)
				loAction:SetValue(false)
				
				if loAction:IsSelectable() then
					local liScore = DiploScore_DemandMilitaryAccess(ai, loTargetCountryTag, ministerTag, loTargetCountryTag)
			...
			and not(ministerCountry:GetFaction() == loTargetCountry:GetFaction())then
				...
				if loAction:IsSelectable() then
					local liScore = DiploScore_NonAgression(ai, ministerTag, loTargetCountryTag, loTargetCountryTag)
			...
			if loReverseRelation:AllowDebts() then
				...
				if loAction:IsSelectable() then
					local liScore = DiploScore_Debt( ai, loTargetCountryTag, ministerTag, loTargetCountryTag )
			...
			if liInfluenceCount > 0 then
				...
				for i = 1, table.getn(loInfluenceCountry) do
					if ministerCountry:IsNeighbour(loInfluenceCountry[i]) then
						-- Set Alignment Factor to 80 to prevent ping pong effect
						if not(Utils.IsFriend(ai, loFaction, loInfluenceCountry[i]:GetCountry(), 80)) then
				...
				if not(lbNeighborCheck) and not(loInfluenceActionWorst == nil) and liInfluenceLeft > 0 then
					if loInfluenceActionWorst:IsSelectable() then
				...
				elseif not(lbNeighborCheck) and not(loInfluenceActionWorst == nil) and liInfluenceLeft <= 0 then
					...
					for i, paramscore in ipairs(loInfluenceScore) do
						if paramscore < liScore then
							-- Cancel old Influence
							liScore = paramscore
				...
				elseif not(loInfluenceAction == nil) and liInfluenceLeft > 0 then
					if loInfluenceAction:IsSelectable() then
				...
				elseif liInfluenceCount > 0 and not(loInfluenceAction == nil) then
					-- Check your current influence scores and compare them to the ones you have
					for i, infparam in ipairs(loInfluenceScore) do
						if loInfluenceActionScore > infparam then
							-- Cancel old Influence
							local loInfluenceActionCancel = CInfluenceNation(ministerTag, loInfluenceCountry[i])
							loInfluenceActionCancel:SetValue(false)
							minister:Propose(loInfluenceActionCancel , 1000 )
...
function CheckInfluenceParameter(vaInfluenceParameter, vsCountryTag)
	...
	for i, param in ipairs(vaInfluenceParameter) do
		if param == vsCountryTag then
			return true
		end
	end
