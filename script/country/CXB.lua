...
function P.Build_Fort(ic, voProductionData, vbGoOver)
	...
	if liYear <= 1938 then
		ic = Support.Build_Fort(ic, voProductionData, 7390, 2, vbGoOver) -- Shizuishan
		ic = Support.Build_Fort(ic, voProductionData, 7404, 2, vbGoOver) -- Liangzhen
	end
...
function P.Call_ForeignMinister_Tick(minister)
	...
	-- If not in a faction already the align with the allies
	--    if Japan is at war with China
	if not(ministerCountry:HasFaction()) then
		local chiTag = CCountryDataBase.GetTag("CHI")
		local loChinaCountry = chiTag:GetCountry()
