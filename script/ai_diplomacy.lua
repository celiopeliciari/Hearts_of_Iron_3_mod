...
function DiploScore_Alliance(voAI, voActorTag, voRecipientTag, voObserverTag)
	if voObserverTag == voActorTag then 
		...
		score = Utils.CallScoredCountryAI(voRecipientTag, 'DiploScore_Alliance', score, voAI, voActorTag, voRecipientTag,voObserverTag)
		if score < 50 then
			score = 0
		end
		return score
...
function DiploScore_Debt(voAI, voActorTag, voRecipientTag, voObserverTag)
	local loRecipientCountry
	local loActorCountry
	
	-- This is a normal Debt Call
	if voObserverTag == voActorTag then
		loActorCountry = voRecipientTag:GetCountry()
		loRecipientCountry = voActorTag:GetCountry()
	
	-- This is a cancel check
	else
		loRecipientCountry = voRecipientTag:GetCountry()
		loActorCountry = voActorTag:GetCountry()
	end
	
	if loRecipientCountry:IsAtWar() 
	and loRecipientCountry:HasFaction()
	and loRecipientCountry:GetFaction() == loActorCountry:GetFaction()
	then
		local loReqMoney = loRecipientCountry:GetPool():Get( CGoodsPool._MONEY_ ):Get()
		local loActMoney = loActorCountry:GetPool():Get( CGoodsPool._MONEY_ ):Get()
		
		-- Make sure they have a money reserve to even allow the debt
		if loReqMoney > loActMoney
		and loReqMoney > 200 
		then
			return 100
		end
